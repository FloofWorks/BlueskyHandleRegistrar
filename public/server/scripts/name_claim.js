...
document.addEventListener('DOMContentLoaded', () => {
        ...
        .then(result => {
            if (result.success) {
                ...
                combinedRegistrants.forEach(registrant => {
                    ...
                    a.target = '_blank'; 
                    a.textContent = `${registrant.username}`;
                    li.appendChild(a);
    ...
    const faqData = [
        ...
        },
        {
            ...
            answer: `<p>Registering a non-pure handle is instant. There is no wait-time attached to registering that kind of handle. <br> If you did register a pure handle, it may take up to a day untill you recieve a response email!</p>
            <p><h6></h6></p>`
        },
        {
            question: "How do i apply my Handle?",
            answer: `<p>After you have registered your handle, and made sure that it is valid, Open Your Bluesky Settings > Change Handle > I have My Own Domain > Enter your full handle into the text box > Verify DNS Record > Save</p>
            <p>If you encounter any errors, please do not hesitate to contact me via </strong>YOUR_EMAIL_HERE</strong></p>`
        },
        {
        ...
        },
        {
            ...
            answer: `<p>After you have registered, you need to manually apply your handle inside bluesky! <br> If your handle was a pure handle, you need to wait for a verification email! <br> If you are unsure,</p><p>you can check your Handle's Status at all times at <a href="https://bsky-debug.app/handle" target="_blank">here!</p>`
        },
        {
        ...
        },
        {
            ...
            answer: `<p>Currently, unlinking and changing handles is not supported through this interface. Please contact us at <a href="mailto:YOUR_EMAIL_HERE">YOUR_EMAIL_HERE</a> for assistance.</p>
        ...
        },
        {
            ...
            <p>If you still have concearns, reach out to me at <strong>floofworks.0+concearn@gmail.com</strong>!</p>`
        }
    ];
    ...
    function setDefaultDomain() {
        ...
        //     secondLevelDomain = hostname;//hostParts[hostParts.length - 2];
        // }

        // const allowedDomains = ['pmd.social', 'lucario.social', 'renamon.social', 'riolu.social']; //< EDIT THIS TO BE YOUR AVAILABLE DOMAINS

        // if (allowedDomains.includes(secondLevelDomain)) {
        //     const domainValue = `${secondLevelDomain}`;
    ...
    function checkDidAvailability() {
        ...
        fetch(`/api/getDataByDid/${encodeURIComponent(did)}`)
            ...
            .then(result => {
                if (result.success) {
                    didStatus.textContent = 'You may only occupy one handle per DID, since unlinking has not been added yet. Please send an email to YOUR_EMAIL_HERE for further assistance.';
